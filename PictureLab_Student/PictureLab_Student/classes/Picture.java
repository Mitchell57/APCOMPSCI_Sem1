...
public class Picture extends SimplePicture 
{
  ...
  public void grayscale()
  {
    ...
    for (Pixel[] rowArray : pixels)
    {
      for (Pixel pixelObj : rowArray)
      {
        int ave = (pixelObj.getRed() + pixelObj.getBlue() + pixelObj.getGreen()) / 3;
		pixelObj.setRed(ave);
  ...
  //Mirrors Top to Bottom instead of Bottom to Top 
  public void mirrorDiagonal()
  {
	...
	for (int row = 0; row < pixels.length; row++)
    {
      for (int col = startY; col < width; col++)
      {
        leftPixel = pixels[row][col];
        if(row<width && col<pixels.length){
			rightPixel = pixels[col][row];
		}
		else{
			rightPixel = pixels[pixels.length-1][width-1];
		}
		rightPixel.setColor(leftPixel.getColor());
  ...
  {
    Picture flower1 = new Picture("snowman.jpg");
    Picture flower2 = new Picture("seagull.jpg");
	flower1.negate();
	flower2.grayscale();
	flower1.zeroBlue();
    this.copy(flower1,0,0, 100, 100, 0, 0);
    this.copy(flower2,50,0, 100, 55, 150, 0);
    this.copy(flower1,50,0, 200, 100, 100, 100);
    this.write("collage.jpg");
  }
    ...
    public void edgeDetection2(int edgeDist)
  {
	...
	for (int row = 0; row < pixels.length-1; row++)
    {
           ...
           col < pixels[0].length; col++)
      {
        ...
        topPixel = pixels[row][col];
        bottomColor = bottomPixel.getColor();
        if (Math.abs(topPixel.getAverage()-bottomPixel.getAverage()) > 
            edgeDist)
          topPixel.setColor(Color.BLACK);
      }
    }
